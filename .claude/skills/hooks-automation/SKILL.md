...
## V3 Hook Categories & 12 Background Workers (migrated from root CLAUDE.md)

> The following was moved out of the root CLAUDE.md. It documents the newer v3 hook command surface (`npx claude-flow@v3alpha hooks <verb>`, plural) and the 12 background workers, which are not covered by this skill's older `npx claude-flow hook <verb>` (singular) command reference above.
